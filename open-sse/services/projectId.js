...
function startCacheCleanup() {
    ...
    _cleanupTimer = setInterval(() => {
        ...
        for (const [connectionId, entry] of projectIdCache) {
            if (now - entry.fetchedAt > CACHE_TTL_MS) {
                projectIdCache.delete(connectionId);
            }
        }

        // 2. Abort and evict stuck in-flight fetches
        for (const [connectionId, entry] of pendingFetches) {
            if (now - entry.startedAt > PENDING_TTL_MS) {
                try { entry.controller.abort(); } catch (_) { /* ignore */ }
 ...
 * @param {string} [provider]   - Provider name ("antigravity" | "gemini-cli")
 * @returns {Promise<string|null>} Real project ID or null
 */
export async function getProjectIdForConnection(connectionId, accessToken, provider = null) {
...
async function fetchProjectId(accessToken, signal, provider) {
    const endpoints = CLOUD_CODE_API?.[provider] || CLOUD_CODE_API?.["gemini-cli"] || CLOUD_CODE_API;
    const loadUrl = endpoints?.loadCodeAssist || CLOUD_CODE_API.loadCodeAssist;
...
async function onboardUser(accessToken, tierID, externalSignal, endpoints, provider) {
    ...
    const headers = provider === "antigravity" ? ANTIGRAVITY_LOAD_CODE_ASSIST_HEADERS : LOAD_CODE_ASSIST_HEADERS;
    const onboardUrl = endpoints?.onboardUser || CLOUD_CODE_API.onboardUser;
