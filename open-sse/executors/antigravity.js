...
} from "../services/antigravityClientProfile.js";
...
export class AntigravityExecutor extends BaseExecutor {
  ...
  async prepareRequest(credentials) {
    const profile = getAntigravityClientProfile(credentials);
    credentials._antigravityClientProfile = profile;
    credentials._antigravityClientVersion = await resolveAntigravityClientVersion(profile);
  }
  ...
  buildHeaders(credentials, stream = true, sessionId = null) {
    const profile = credentials?._antigravityClientProfile || getAntigravityClientProfile(credentials);
    return getAntigravityContentHeaders(
      profile,
      credentials.accessToken,
      credentials?._antigravityClientVersion,
    );
  }
