---
name: ktrouter-stt
description: Speech-to-text via KTRouter /v1/audio/transcriptions using OpenAI Whisper / Groq / Gemini / Deepgram / AssemblyAI / NVIDIA / HuggingFace models. Use when the user wants to transcribe audio, convert speech to text, or get subtitles from audio files.
---

# KTRouter — Speech-to-Text
...
```bash
curl $KTROUTER_URL/v1/models/stt | jq '.data[].id'
# Per-model params (language, response_format, prompt, temperature support)
curl "$KTROUTER_URL/v1/models/info?id=openai/whisper-1"
```
...
`POST $KTROUTER_URL/v1/audio/transcriptions` (OpenAI Whisper compatible, `multipart/form-data`)
...
```bash
curl -X POST "$KTROUTER_URL/v1/audio/transcriptions" \
...
const r = await fetch(`${process.env.KTROUTER_URL}/v1/audio/transcriptions`, {
  method: "POST",
  headers: { "Authorization": `Bearer ${process.env.KTROUTER_KEY}` },
  body: form,
});
