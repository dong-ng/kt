# KTRouter Architecture

_Last updated: 2026-02-06_

## Executive Summary

KTRouter is a local AI routing gateway and dashboard built on Next.js.
...
flowchart LR
    ...
    subgraph Router[KTRouter Local Process]
...
- `src/lib/localDb.js`
- file: `${DATA_DIR}/db.json` (or `~/.ktrouter/db.json` when `DATA_DIR` is unset)
- entities: providerConnections, providerNodes, modelAliases, combos, apiKeys, settings, pricing

Usage DB:

- `src/lib/usageDb.js`
- files: `~/.ktrouter/usage.json`, `~/.ktrouter/log.txt`
...
- main state: `${DATA_DIR}/db.json` (or `~/.ktrouter/db.json`)
- usage stats: `~/.ktrouter/usage.json`
- request log lines: `~/.ktrouter/log.txt`
...
flowchart LR
    ...
    subgraph ContainerOrProcess[KTRouter Runtime]
        Next[Next.js Server\nPORT=3008]
...
1. `usageDb` currently stores under `~/.ktrouter` and does not follow `DATA_DIR`.
...
- Build from source: `cd /root/dev/ktrouter && npm run build`
- Build Docker image: `cd /root/dev/ktrouter && docker build -t ktrouter .`
- Start service and verify:
- `GET /api/settings`
- `GET /api/v1/models`
- CLI target base URL should be `http://<host>:3008/v1` when `PORT=3008`
