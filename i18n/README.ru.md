<div align="center">
  <img src="../images/ktrouter.png?1" alt="Панель управления KTRouter" width="800"/>
  
  # KTRouter - Free AI Router
  ...
  **Бесплатный AI-провайдер для OpenClaw.**
  ...
  [🚀 Быстрый старт](#-quick-start) • [💡 Возможности](#-key-features) • [📖 Установка](#-setup-guide) • [🌐 Сайт](https://ktrouter.com)
</div>
...
🎉 Панель управления откроется на `http://localhost:3008`
...
Пакет этого репозитория приватный (`ktrouter`), поэтому запуск из исходников/Docker — это ожидаемый путь локальной разработки.
...
URL по умолчанию:
- Панель управления: `http://localhost:3008/dashboard`
- OpenAI-совместимый API: `http://localhost:3008/v1`
...
### 📺 Полное руководство по настройке - KTRouter + Claude Code БЕСПЛАТНО
  
[![Настройка KTRouter + Claude Code](https://img.youtube.com/vi/raEyZPg5xE0/maxresdefault.jpg)](https://www.youtube.com/watch?v=raEyZPg5xE0)

**🎬 Полное пошаговое руководство:**
- ✅ Установка и настройка KTRouter
...
KTRouter бесшовно работает со всеми основными AI-инструментами для кодинга:
...
- Ваш CLI-инструмент отправляет формат OpenAI → KTRouter транслирует → Провайдер получает родной формат
...
> Сам KTRouter **никогда ничего не взимает** с вас. Вы платите напрямую провайдерам (если используете платные сервисы).
> 
...
### 📊 Понимание затрат и оплаты в KTRouter

**Реальность оплаты KTRouter:**

✅ **Софт KTRouter = БЕСПЛАТНО навсегда** (открытый код, никогда не взимает плату)  
...
❌ **KTRouter никогда не выставляет счёт** и не списывает с вашей карты
...
- **Дешёвые провайдеры** (GLM, MiniMax): Платите им напрямую, KTRouter только маршрутизирует
- **БЕСПЛАТНЫЕ провайдеры** (iFlow, Kiro, Qwen): Действительно бесплатны навсегда, без скрытых платежей
- **KTRouter**: Никогда ничего не взимает, никогда
...
Панель отслеживает ваше использование токенов и показывает **оценочные затраты**, как если бы вы напрямую использовали платный API. Это **не реальная оплата** — это справка, показывающая, сколько вы экономите, используя бесплатные модели или существующие подписки через KTRouter.
...
<details>
<summary><b>💳 Взимает ли с меня плату KTRouter?</b></summary>

**Нет.** KTRouter — это бесплатное ПО с открытым кодом, работающее на вашем собственном компьютере. Оно никогда ничего с вас не взимает.

**Вы платите только:**
- ✅ **Провайдерам подписки** (Claude Code $20/мес, Codex $20-200/мес) → Платите им напрямую на их сайте
- ✅ **Дешёвым провайдерам** (GLM, MiniMax) → Платите им напрямую, KTRouter только маршрутизирует ваши запросы
- ❌ **Самому KTRouter** → **Никогда ничего не взимает, никогда**

KTRouter — это локальный прокси/роутер. У него нет вашей кредитной карты, он не может выставлять счета и не имеет платёжной системы. Это полностью бесплатное ПО.
...
KTRouter только маршрутизирует ваши запросы к ним — никаких «ловушек» или будущих платежей. Это действительно бесплатные сервисы, а KTRouter облегчает их использование с поддержкой резервирования.

**Примечание:** Некоторые провайдеры подписки (Antigravity, GitHub Copilot) могут иметь бесплатные пробные периоды, которые позже становятся платными, но об этом чётко уведомляют сами провайдеры, а не KTRouter.
...
3. **Используйте провайдеров подписки в последнюю очередь:**
   ...
   - KTRouter помогает максимизировать их ценность через отслеживание квоты
...
Умный механизм резервирования KTRouter предотвращает неожиданные расходы:
...
**Вы контролируете:** Установите лимиты расходов на каждого провайдера в панели, и KTRouter будет их соблюдать.
...
**Профи-совет:** Используйте Opus для сложных задач, Sonnet для скорости. KTRouter отслеживает квоту для каждой модели!
...
Settings → Models → Advanced:
  ...
  OpenAI API Key: [из панели управления ktrouter]
...
> **Примечание:** OpenClaw работает только с локальным KTRouter. Используйте `127.0.0.1` вместо `localhost`, чтобы избежать проблем с разрешением имён.
...
docker run -d \
  --name ktrouter \
  -p 3008:3008 \
  --env-file /root/dev/ktrouter/.env \
  -v ktrouter-data:/app/data \
  -v ktrouter-usage:/root/.ktrouter \
  ktrouter
```
...
|----------|---------|-------------|
| `JWT_SECRET` | Автогенерация (`~/.ktrouter/jwt-secret`) | Секрет подписи JWT для cookie аутентификации панели (задайте для общего доступа между инстансами) |
| `INITIAL_PASSWORD` | `123456` | Пароль первого входа при отсутствии сохранённого хеша |
| `DATA_DIR` | `~/.ktrouter` | Расположение основной БД приложения (`db.json`) |
| `PORT` | framework default | Порт сервиса (`3008` в примерах) |
...
| `BASE_URL` | `http://localhost:3008` | Внутренний серверный базовый URL для задач облачной синхронизации |
| `CLOUD_URL` | `https://ktrouter.com` | Серверный базовый URL эндпоинта облачной синхронизации |
...
| `NEXT_PUBLIC_CLOUD_URL` | `https://ktrouter.com` | Обратно совместимый/публичный облачный URL (приоритет `CLOUD_URL` для серверного рантайма) |
...
- История использования и логи: `~/.ktrouter/usage.json` и `~/.ktrouter/log.txt`, управляется `src/lib/usageDb.js`.
- Опциональные логи запросов/транслятора: `<repo>/logs/...` при `ENABLE_REQUEST_LOGS=true`.
- Хранилище использования следует логике пути `~/.ktrouter` и независимо от `DATA_DIR`.
...
- Установите `PORT=3008` и `NEXT_PUBLIC_BASE_URL=http://localhost:3008`

**Ошибки облачной синхронизации**
- Убедитесь, что `BASE_URL` указывает на ваш работающий инстанс (например, `http://localhost:3008`)
- Убедитесь, что `CLOUD_URL` указывает на ожидаемый облачный эндпоинт (например, `https://ktrouter.com`)
...
- Симптом обычно появляется на публичном облачном эндпоинте (`https://ktrouter.com/v1`) для непотоковых (non-streaming) вызовов.
...
- Локальный рантайм KTRouter включает резервирование SSE→JSON для непотоковых вызовов, когда upstream возвращает `text/event-stream`.
...
- `tester/security/test-cloud-openai-compatible.sh`
  - Отправляет OpenAI-совместимый запрос напрямую на облачный эндпоинт (`https://ktrouter.com/v1/chat/completions`) с указанной моделью/ключом.
...
- Локально (`http://127.0.0.1:3008/v1/chat/completions`): работает с `stream=false` и `stream=true`.
...
- Публичный облачный эндпоинт (`https://ktrouter.com/v1/chat/completions`):
...
- **Сайт**: [ktrouter.com](https://ktrouter.com)
...
Спасибо всем, кто помогает делать KTRouter лучше!

[![Contributors](https://contrib.rocks/image?repo=dongbao492/ktrouter&max=100&columns=20&anon=1)](https://github.com/dongbao492/ktrouter/graphs/contributors)
...
См. [Pull Requests](https://github.com/dongbao492/ktrouter/pulls) для подробных инструкций.

---

## 🔀 Форки

**[OmniRoute](https://github.com/diegosouzapw/OmniRoute)** — Полнофункциональный TypeScript-форк KTRouter. Добавляет 36+ провайдеров, авторезервирование на 4 уровнях, мультимодальный API (изображения, embedding, аудио, TTS), circuit breaker, семантическое кеширование, оценку LLM и доработанную панель. 368+ юнит-тестов. Доступен через npm.
