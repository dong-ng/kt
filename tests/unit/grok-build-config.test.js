...
describe("grokBuildConfig", () => {
  it("creates independent main and per-type subagent model slots", () => {
    ...
    expect(parsed.default).toBe("ktrouter");
    expect(parsed.model).toMatchObject({
      model: "cx/gpt-5.6-sol",
      base_url: "http://127.0.0.1:3008/v1",
      context_window: 400000,
    });
    expect(parsed.subagentMappings).toMatchObject({
      "general-purpose": "ktrouter-general-purpose",
      explore: "ktrouter-explore",
      plan: "grok-4.5",
    });
  ...
  it("is idempotent and updates owned slots without duplicate sections", () => {
    ...
    expect(result.match(/^\[model\.ktrouter\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.ktrouter-general-purpose\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.ktrouter-explore\]$/gm)).toHaveLength(1);
    expect(result.match(/^# ktrouter-prev-subagent-explore/gm)).toHaveLength(1);
  ...
  it("blank override restores previous subagent mapping and removes owned slot", () => {
    ...
    expect(result).not.toContain("[model.ktrouter-explore]");
    expect(parsed.subagentMappings["general-purpose"]).toBe("ktrouter-general-purpose");
  });
  ...
  it("reset restores previous default and all previous subagent mappings", () => {
    ...
    expect(reset).not.toContain("[model.ktrouter-");
    expect(reset).not.toContain("ktrouter-prev-");
  ...
  it("removes mappings that were originally unset", () => {
    ...
    expect(parseGrokBuildConfig(applied).subagentMappings.plan).toBe("ktrouter-plan");
  ...
  it("legacy callers without subagentModels leave existing overrides untouched", () => {
    ...
    expect(parsed.subagentMappings.explore).toBe("ktrouter-explore");
  ...
  it("returns stable slot names only for supported subagent types", () => {
    expect(getGrokSubagentSlot("general-purpose")).toBe("ktrouter-general-purpose");
    expect(getGrokSubagentSlot("explore")).toBe("ktrouter-explore");
    expect(getGrokSubagentSlot("plan")).toBe("ktrouter-plan");
