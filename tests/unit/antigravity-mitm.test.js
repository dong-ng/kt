...
const { MODEL_NO_MAP, MODEL_PATTERNS, MODEL_SYNONYMS } = require("../../src/mitm/config.js");
...
describe("Antigravity MITM model handling", () => {
  ...
  it("does not exclude real agent models from re-routing", () => {
    for (const id of ["gemini-3.6-flash-high", "gemini-3.5-flash-low", "gemini-3-flash-agent", "claude-sonnet-4-6"]) {
      expect((MODEL_NO_MAP.antigravity || []).some((re) => re.test(id))).toBe(false);
    }
  });

  it("exposes every new live Antigravity model as a separate mapping slot", () => {
    for (const id of [
      ...
      expect(ag.defaultModels.some((model) => model.alias === id)).toBe(true);
    }
  });
  ...
  ])("matches '%s' to its own MITM slot", (id, expectedAlias) => {
    const matched = MODEL_PATTERNS.antigravity.find(({ match }) => match.test(id));
    expect(matched?.alias).toBe(expectedAlias);
  });
});
