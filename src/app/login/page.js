...
export default function LoginPage() {
  ...
  useEffect(() => {
    async function checkAuth() {
      ...
      try {
        const res = await fetch(`${baseUrl}${AUTH_STATUS_PATH}`, {
        ...
        if (res.ok) {
          ...
          // Sync custom router name from settings via auth status
    ...
    <div className="min-h-screen flex items-center justify-center bg-bg p-4 relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-md">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-primary mb-2">{routerName}</h1>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  {resetHint && (
                    <p className="text-xs text-text-muted">
                      Forgot password? Open <code className="bg-sidebar px-1 rounded">ktrouter</code> CLI on the host → <b>Settings</b> → <b>Reset Password to Default</b>.
                    </p>
                  )}
                </div>
