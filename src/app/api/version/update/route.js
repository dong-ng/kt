...
export async function POST() {
  if (process.env.NODE_ENV !== "production") {
    return NextResponse.json(
      { success: false, message: "Update is only available in production build (ktrouter CLI)" },
      { status: 403 }
    );
  }

  try {
    // Kill sibling processes (MITM and stray next-server) to release file locks on Windows.
