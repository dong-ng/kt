...
const hasKTRouterConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "KTRouter");
};

const getKTRouterEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "KTRouter") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getKTRouterEntry(config);
...
// POST - Apply KTRouter config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing KTRouter entry or append
    const idx = config.findIndex((e) => e.name === "KTRouter");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove KTRouter entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "KTRouter");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "KTRouter removed from Copilot config",
    });
  } catch (error) {
