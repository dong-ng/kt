...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-ktrouter.env");
};
...
const hasKTRouterConfig = (config) => {
  ...
  if (providers["ktrouter"]) return true;

  for (const [name, provider] of Object.entries(providers)) {
    if (provider.base_url && provider.base_url.includes("localhost:3008")) {
      return true;
    }
  }
...
export async function GET() {
  ...
  const hasKTRouter = hasKTRouterConfig(config);
...
export async function POST(request) {
  try {
    ...
    env.JCODE_KTROUTER_API_KEY = apiKey;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "jcode configured successfully. Use: jcode --provider-profile ktrouter",
      configPath: getConfigPath(),
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    delete config.providers["ktrouter"];

    await writeConfig(config);

    const env = await readProviderEnv();
    delete env.JCODE_KTROUTER_API_KEY;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "ktrouter configuration removed from jcode",
    });
  } catch (error) {
