...
// Check if settings has KTRouter config
const hasKTRouterConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["ktrouter"];
};

// Read per-agent models.json and return current model id (without "ktrouter/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["ktrouter"]?.models;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasKTRouter: hasKTRouterConfig(settings),
...
// POST - Update KTRouter settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `ktrouter/${model}`;

    // Remove all old ktrouter/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("ktrouter/"))
    ...
    // Add fresh ktrouter models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`ktrouter/${m}`] = {};
    });

    // Remove old ktrouter model from each agent in agents.list. The
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("ktrouter/")) {
    ...
    // Update models.providers.ktrouter with all models
    settings.models.providers["ktrouter"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `ktrouter/${agentModel}` };
        return agent;
      });
...
export async function DELETE() {
  try {
    ...
    // Remove KTRouter from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["ktrouter"];
    ...
    // Remove ktrouter models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("ktrouter/"));
    ...
    // Reset agents.defaults.model.primary if it uses ktrouter
    if (settings.agents?.defaults?.model?.primary?.startsWith("ktrouter/")) {
