...
// Check if settings has KTRouter customModels
const hasKTRouterConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id?.startsWith("custom:KTRouter"));
};
...
// POST - Update KTRouter customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing KTRouter configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:KTRouter"));
...
// DELETE - Remove KTRouter customModels only (keep other settings)
