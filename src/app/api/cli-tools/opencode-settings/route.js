...
const hasKTRouterConfig = (config) => {
  ...
  return !!config.provider["ktrouter"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["ktrouter"];
    ...
    return NextResponse.json({
      ...
      configPath: getConfigPath(),
        opencode: {
          ...
          activeModel: config?.model?.startsWith("ktrouter/") ? config.model.replace(/^ktrouter\//, "") : null,
...
// POST - Apply KTRouter as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_ktrouter";
    const effectiveSubagentModel = subagentModel || modelsArray[0];

    // Ensure provider object
    if (!config.provider) config.provider = {};

    // Preserve any existing ktrouter provider entry and its models
    const existingProvider = config.provider["ktrouter"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["ktrouter"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `ktrouter/${finalActive}`;
      }
    }
    ...
    config.agent.explorer = {
      ...
      mode: "subagent",
      model: `ktrouter/${effectiveSubagentModel}`,
    };
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("ktrouter/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove KTRouter provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["ktrouter"]?.models) {
      delete config.provider["ktrouter"].models[modelToRemove];
      
      // If no models left, remove the provider
      if (Object.keys(config.provider["ktrouter"].models).length === 0) {
        delete config.provider["ktrouter"];
        if (config.model?.startsWith("ktrouter/")) delete config.model;
      } else if (config.model === `ktrouter/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["ktrouter"].models);
        config.model = `ktrouter/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire ktrouter provider
      if (config.provider) delete config.provider["ktrouter"];
      if (config.model?.startsWith("ktrouter/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("ktrouter/")) {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "KTRouter settings removed from OpenCode",
    });
  } catch (error) {
