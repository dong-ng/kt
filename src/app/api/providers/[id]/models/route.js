...
import { resolveLiveProviderModels } from "open-sse/services/liveProviderModels.js";
import { resolveCopilotModels } from "open-sse/services/copilotModels.js";
import { getAntigravityContentHeaders } from "open-sse/services/antigravityHeaders.js";
import {
...
const LIVE_SYNC_PROVIDERS = new Set(["antigravity", "github", "codex"]);
...
const parseGeminiCliModels = (data) => {
  if (Array.isArray(data?.models)) {
    return data.models
      .filter((item) => !item?.isInternal && !/^tab[_-]/i.test(item?.id || item?.model || item?.name || ""))
      .map((item) => {
  ...
  if (data?.models && typeof data.models === "object") {
    return Object.entries(data.models)
      .filter(([id, info]) => !info?.isInternal && !/^tab[_-]/i.test(id))
      .map(([id, info]) => ({
        id,
...
const PROVIDER_MODELS_CONFIG = {
  ...
  antigravity: {
    customResolver: buildOAuthResolver({
      refreshFn: (conn) => refreshGoogleToken(
        conn.refreshToken,
        GEMINI_CONFIG.clientId,
        GEMINI_CONFIG.clientSecret,
      ),
      fetchFn: async (token, connection) => {
        const profile = getAntigravityClientProfile(connection);
        ...
        return fetch(GEMINI_CLI_MODELS_URL, {
          method: "POST",
          headers: {
            ...getAntigravityContentHeaders(profile, token, clientVersion),
  ...
  github: {
    customResolver: async (connection, options = {}) => {
      const result = await resolveCopilotModels(connection, {
      ...
      return result || { models: [], warning: "GitHub Copilot returned no live models" };
    },
  },
...
export async function GET(request, { params }) {
  try {
    ...
    if (typeof config.customResolver === "function") {
      ...
      const result = LIVE_SYNC_PROVIDERS.has(connection.provider)
        ? await resolveLiveProviderModels({
            provider: connection.provider,
            connectionId: connection.id,
            staticModels: getStaticProviderModels(connection.provider),
            forceRefresh,
            loadModels: async () => {
              const loaded = await config.customResolver(connection, { forceRefresh });
              if (loaded?.error) throw new Error(loaded.error);
              return loaded;
            },
          })
        : await config.customResolver(connection);
      if (result.error) {
    ...
    if (config.body && config.method === "POST") {
      fetchOptions.body = JSON.stringify(
        typeof config.body === "function" ? config.body(connection) : config.body
      );
    }
