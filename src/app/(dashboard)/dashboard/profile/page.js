...
import { useRouterName, useRouterNameSettings } from "@/shared/hooks/useRouterName";
...
export default function ProfilePage() {
  ...
  const [routerNameInput, setRouterNameInput] = useState("");
  const [routerNameStatus, setRouterNameStatus] = useState({ type: "", message: "" });
  const [routerNameLoading, setRouterNameLoading] = useState(false);
  ...
  useEffect(() => {
    fetch("/api/settings")
      ...
      .then((data) => {
        ...
        setRouterNameInput(data?.customName || "");
  ...
  const handleExportDatabase = async (password) => {
    ...
    try {
      const res = await fetch("/api/settings/database", {
        headers: { "x-ktrouter-password": password },
      });
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
      ...
      anchor.href = url;
      anchor.download = `ktrouter-backup-${stamp}.json`;
      ...
      <div className="flex flex-col gap-6">
        ...
        <Card>
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h3 className="text-base sm:text-lg font-semibold">Router Name</h3>
              <p className="text-xs text-text-muted">Customize how your router appears in the UI</p>
            </div>
          </div>
          <div className="flex flex-col gap-3">
            <div className="flex flex-col sm:flex-row gap-2">
              <Input
                ...
                value={routerNameInput}
                onChange={(e) => setRouterNameInput(e.target.value)}
                disabled={routerNameLoading}
                className="flex-1"
              />
              <div className="flex gap-2">
                <Button
                  variant="primary"
                  loading={routerNameLoading}
                  className="w-full sm:w-auto"
                  onClick={async () => {
                    setRouterNameLoading(true);
                    setRouterNameStatus({ type: "", message: "" });
                    try {
                      const res = await fetch("/api/settings", {
                        method: "PATCH",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({ customName: routerNameInput.trim() }),
                      });
                      if (res.ok) {
                        const data = await res.json();
                        setSettings((prev) => ({ ...prev, ...data }));
                        setCustomName(data.customName || "");
                        setRouterNameStatus({ type: "success", message: routerNameInput.trim() ? "Router name updated" : "Reset to default" });
                      } else {
                        const data = await res.json();
                        setRouterNameStatus({ type: "error", message: data.error || "Failed to update" });
                      }
                    } catch {
                      setRouterNameStatus({ type: "error", message: "An error occurred" });
                    } finally {
                      setRouterNameLoading(false);
                    }
                  }}
                >
                  Save
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        if (res.ok) {
                          ...
                          setSettings((prev) => ({ ...prev, ...data }));
                          setCustomName("");
                          setRouterNameStatus({ type: "success", message: "Reset to default" });
                        }
                      } catch {
                        setRouterNameStatus({ type: "error", message: "An error occurred" });
                      } finally {
            ...
            {routerNameStatus.message && (
              <p className={`text-xs sm:text-sm ${routerNameStatus.type === "error" ? "text-red-500" : "text-green-500"}`}>
                {routerNameStatus.message}
              </p>
            )}
            <p className="text-xs text-text-muted">
              Currently showing as: <span className="font-semibold text-text-main">{routerName}</span>. Leave empty to use default ({APP_CONFIG.name}).
            </p>
          </div>
        </Card>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-3 rounded-lg bg-bg border border-border gap-2">
              <div>
                ...
                <p className="text-xs sm:text-sm text-text-muted font-mono break-all">
                  {process.env.NODE_ENV === "development" ? "./data/db/data.sqlite" : "~/.ktrouter/db/data.sqlite"}
                </p>
              </div>
            </div>
            <div className="flex flex-col sm:flex-row gap-2">
          ...
          <div className="flex flex-col gap-4 mt-4">
            ...
            <div className="grid grid-cols-1 gap-4">
              <div className="flex flex-col gap-2">
                ...
                <Input
                  placeholder="https://auth.example.com/application/o/ktrouter/"
              ...
              <div className="flex flex-col gap-2">
                ...
                <Input
                  placeholder="ktrouter-dashboard"
        ...
        <div className="text-center text-xs sm:text-sm text-text-muted py-4">
          <p>{routerName} v{APP_CONFIG.version}</p>
