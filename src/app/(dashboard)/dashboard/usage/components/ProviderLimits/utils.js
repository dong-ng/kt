...
export function getConnectionLabel(connection) {
  if (connection.provider === "github") {
    return connection.providerSpecificData?.githubLogin?.trim()
...
export function sortVisibleConnections(
  ...
  const compareProviderAndLabel = (a, b) =>
  ...
  if (!expiringFirst) return [...connections].sort(compareProviderAndLabel);
  ...
  return [...connections].sort((a, b) => {
    ...
    return compareProviderAndLabel(a, b);
  });
}
        ...
        if (data.quotas) {
          ...
          Object.entries(data.quotas).forEach(([name, quota]) => {
            normalizedQuotas.push({
              name: displayNames[name] || name.replaceAll("_", " "),
              used: quota.used || 0,
              total: quota.total || 0,
