...
export default function ConnectionRow({ connection, proxyPools, isOAuth, isFirst, isLast, onMoveUp, onMoveDown, onToggleActive, onUpdateProxy, onEdit, onDelete, oneByOneStatus = null, autoPing = null }) {
  ...
  const githubLogin = connection.provider === "github"
    ? connection.providerSpecificData?.githubLogin?.trim()
    : null;
  ...
  const secondaryDisplayName = connection.provider === "github"
    ? [
        connection.providerSpecificData?.githubName?.trim(),
        connection.providerSpecificData?.githubEmail?.trim(),
      ].find((value) => value && value !== displayName) || null
    : connection.name?.trim() && connection.email?.trim() && connection.name.trim() !== connection.email.trim()
      ? connection.email.trim()
    ...
    <div className={`group flex min-w-0 flex-col gap-3 rounded-lg p-2 transition-colors hover:bg-black/[0.02] dark:hover:bg-white/[0.02] sm:flex-row sm:items-center sm:justify-between ${connection.isActive === false ? "opacity-60" : ""}`}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mt-1 flex min-w-0 flex-wrap items-center gap-1.5 sm:gap-2">
            ...
            {connection.provider === "antigravity" && (
              <Badge variant="default" size="sm">
                {connection.providerSpecificData?.clientProfile === "cli" ? "CLI" : "IDE"}
              </Badge>
            )}
            {hasAnyProxy && (
      ...
      <div className="flex w-full items-center justify-between gap-2 sm:w-auto sm:justify-end">
        <div className="grid flex-1 grid-cols-3 gap-1 sm:flex sm:flex-none">
          ...
          <button
            onClick={() => window.open(`/api/providers/export?connectionId=${encodeURIComponent(connection.id)}`, "_blank")}
            ...
            title="Export this connection as JSON"
          >
