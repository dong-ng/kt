...
const LIVE_MODEL_PROVIDER_IDS = new Set(["antigravity", "github", "codex", "cursor"]);
...
export default function ProviderDetailPage() {
  ...
  const importConnectionsInputRef = useRef(null);
  ...
  const [liveModelsMeta, setLiveModelsMeta] = useState(null);
  const [liveModelsRefreshKey, setLiveModelsRefreshKey] = useState(0);
  ...
  const models = LIVE_MODEL_PROVIDER_IDS.has(providerId) && liveModels.length > 0
    ? liveModels
    : staticModels;
  ...
  const handleImportConnections = async (event) => {
    ...
    try {
      ...
      const response = await fetch(`/api/providers/import?provider=${encodeURIComponent(providerId)}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (result.skipped > 0) {
        setAddConnectionError(`Imported ${result.imported}; skipped ${result.skipped}.`);
      }
    } catch (error) {
      setAddConnectionError(error.message || "Import failed");
    } finally {
  ...
  // Account-scoped catalogs change frequently for these providers. The API
  // caches successful responses and returns the static registry as fallback.
  useEffect(() => {
    if (!LIVE_MODEL_PROVIDER_IDS.has(providerId)) {
      setLiveModels([]);
      setLiveModelsMeta(null);
      return;
    }
    ...
    let cancelled = false;
    setLiveModelsMeta((current) => ({ ...current, loading: true }));
    const refresh = liveModelsRefreshKey > 0 ? "?refresh=1" : "";
    fetch(`/api/providers/${connection.id}/models${refresh}`, { cache: "no-store" })
      ...
      .then(({ ok, data }) => {
        if (!cancelled && ok && Array.isArray(data.models) && data.models.length > 0) {
          ...
          setLiveModelsMeta({ source: "static", warning: data.error || "Live models unavailable", loading: false });
        }
      })
      .catch(() => {
        if (!cancelled) setLiveModelsMeta({ source: "static", warning: "Live models unavailable", loading: false });
      });

    return () => { cancelled = true; };
  }, [providerId, connections, liveModelsRefreshKey]);
        ...
        <Card>
          ...
          <div className="mb-3 flex flex-wrap gap-2">
            ...
            <Button
              ...
              variant="secondary"
              icon="upload"
              onClick={() => importConnectionsInputRef.current?.click()}
              disabled={importingConnections}
            >
              {importingConnections ? "Importing..." : "Import"}
            </Button>
            <Button
              size="sm"
              variant="secondary"
              icon="download"
              onClick={() => window.open(`/api/providers/export?provider=${encodeURIComponent(providerId)}`, "_blank")}
              disabled={connections.length === 0}
            >
              Export All
            </Button>
            {addConnectionError && (
              <span className="self-center text-xs text-red-500">{addConnectionError}</span>
            )}
          </div>
      ...
      <Card>
        <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            {LIVE_MODEL_PROVIDER_IDS.has(providerId) && liveModelsMeta && (
              <div className="flex items-center gap-1.5" data-i18n-skip="true">
                <Badge
                  variant={liveModelsMeta.source === "live" || liveModelsMeta.source === "live-cache" ? "success" : "default"}
                  size="sm"
                >
                  {liveModelsMeta.loading
                    ? "Syncing"
                    : liveModelsMeta.source === "live"
                      ? "Live"
                      : liveModelsMeta.source === "live-cache"
                        ? "Cached"
                        : liveModelsMeta.source === "stale-cache"
                          ? "Stale cache"
                          : "Static fallback"}
                </Badge>
                <button
                  type="button"
                  onClick={() => setLiveModelsRefreshKey((value) => value + 1)}
                  disabled={liveModelsMeta.loading}
                  title={liveModelsMeta.warning || "Refresh live models"}
                  className="rounded p-1 text-text-muted transition-colors hover:bg-primary/10 hover:text-primary disabled:opacity-50"
                >
