...
export default function CodexToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, cloudEnabled, initialStatus }) {
  ...
  const handleApplySettings = async () => {
    ...
    setMessage(null);
    try {
      // Use sk_ktrouter for localhost if no key, otherwise use selected key
  ...
  const getManualConfigs = () => {
    ...
    const configContent = `# KTRouter Configuration for Codex CLI
model = "${selectedModel}"
model_provider = "ktrouter"

[model_providers.ktrouter]
name = "KTRouter"
