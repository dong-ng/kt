...
const DEFAULT_MITM_ROUTER_BASE = "http://localhost:3008";
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="px-2 py-2 rounded-lg bg-surface/50 border border-border/50 flex flex-col gap-2">
            <p className="text-[11px] text-text-muted leading-relaxed">
              <span className="font-medium text-text-main">Purpose:</span> Use Antigravity IDE & GitHub Copilot → with ANY provider/model from KTRouter
            </p>
            <p className="text-[11px] text-text-muted leading-relaxed">
              <span className="font-medium text-text-main">How it works:</span> Antigravity/Copilot IDE request → DNS redirect to localhost:443 → MITM proxy intercepts → KTRouter → response to Antigravity/Copilot
            </p>
            {status?.antigravityVersion && (
              <div className="flex flex-wrap items-center gap-x-3 gap-y-1 border-t border-border/50 pt-2 text-[11px] text-text-muted" data-i18n-skip="true">
                <span>
                  <span className="font-medium text-text-main">Antigravity IDE:</span>{" "}
                  {status.antigravityVersion.version}
                </span>
                <span title={status.antigravityVersion.sourceUrl}>
                  Feed: {status.antigravityVersion.sourceVersion || "unavailable"}
                </span>
                <Badge
                  variant={status.antigravityVersion.fallbackUsed ? "default" : "success"}
                  size="sm"
                >
                  {status.antigravityVersion.fallbackUsed ? "Fallback" : "Release feed"}
                </Badge>
                {status.antigravityVersion.fetchedAt && (
                  <span title={new Date(status.antigravityVersion.fetchedAt).toLocaleString()}>
                    Checked {new Date(status.antigravityVersion.fetchedAt).toLocaleTimeString()}
                  </span>
                )}
              </div>
            )}
          ...
          <div className="flex flex-col gap-2">
            <div className="grid gap-1 sm:grid-cols-[8rem_auto_1fr] sm:items-center sm:gap-2">
              <span className="text-xs font-semibold text-text-main sm:text-right sm:text-sm">KTRouter Base URL</span>
            ...
            {!isRunning && (
              <div className="grid gap-1 sm:grid-cols-[8rem_auto_1fr] sm:items-center sm:gap-2">
                ...
                <input
                  ...
                  placeholder={cloudEnabled ? "Enter or pick API key" : "sk_ktrouter (default)"}
          ...
          {serverIsWindows && !isAdmin && (
            <div className="flex items-center gap-2 px-2 py-1.5 rounded text-xs bg-red-500/10 text-red-600 border border-red-500/20">
              ...
              <span>Administrator required — restart KTRouter as Administrator to use MITM</span>
            </div>
          )}
        </div>
      </Card>
