...
const CLI_TOKEN_HEADER = "x-ktrouter-cli-token";
const CLI_TOKEN_SALT = "ktrouter-cli-auth";
...
export function isLocalRequest(request) {
  ...
  if (request.headers.get("x-ktrouter-via-proxy")) return false;
  // Trusted peer IP from TCP socket (custom-server.js); unspoofable. Primary anchor for "local".
  const realIp = request.headers.get("x-ktrouter-real-ip");
  if (realIp) {
...
export async function proxy(request) {
  ...
  if (pathname.startsWith("/dashboard")) {
    ...
    try {
      ...
      // On error, keep the secure default and require login.
    }
