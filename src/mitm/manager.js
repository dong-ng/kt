...
// (prevents EBUSY on `npm i -g ktrouter@latest` while MITM is running).
...
const ENCRYPT_SALT = "ktrouter-mitm-pwd";
        ...
        (res) => {
          ...
          res.on("end", () => {
            try {
              ...
              resolve(json.ok === true ? { ...json, ok: true, pid: json.pid || null } : null);
            } catch { resolve(null); }
          });
        }
      );
...
async function getMitmStatus() {
  ...
  if (running) {
    ...
    antigravityVersion = health?.antigravityVersion || null;
  }

  return { running, pid, certExists, certTrusted, dnsStatus, antigravityVersion };
}
...
async function startServer(apiKey, sudoPassword, forceKillPort443 = false) {
  ...
  if (!effectiveServerPath || !fs.existsSync(effectiveServerPath)) {
    ...
    if (!effectiveServerPath || !fs.existsSync(effectiveServerPath)) {
      throw new Error(`MITM server.js not found at ${effectiveServerPath}. Reinstall ktrouter.`);
    }
  }
