...
// Warm the version cache without delaying MITM startup. Requests use the safe
// fallback until the official Antigravity release feed responds.
resolveAntigravityIdeVersion().catch(() => {});
...
function getMappedModel(tool, model) {
  ...
  try {
    ...
    for (const { match, alias } of patterns) {
      // The first matching family is authoritative. Falling through when its slot is
      // empty can route a new model through an older broad family (for example 3.6
      // Flash through the legacy 3.5 Flash slot).
      if (match.test(lookup)) return aliases[alias] || null;
    }
    return null;
  } catch { return null; }
}
  ...
  try {
    if (req.url === "/_mitm_health") {
      ...
      res.end(JSON.stringify({
        ok: true,
        pid: process.pid,
        antigravityVersion: getAntigravityIdeVersionStatus(),
      }));
      return;
    }
    ...
    // Anti-loop: skip requests from KTRouter
