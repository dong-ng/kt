  ...
  try {
    const mitmPidFile = path.join(
      process.platform === "win32"
        ? path.join(process.env.APPDATA || "", "ktrouter")
        : path.join(os.homedir(), ".ktrouter"),
      "mitm",
      ".mitm.pid"
    );
...
// Collect PIDs of all ktrouter-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from ktrouter install dir or wrapper cli.js
        const isAppProcess = lower.includes("ktrouter") ||
    ...
    // Kill tray binaries that hold the app directory lock.
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("ktrouter") ||
...
function getDataDir() {
  ...
  if (process.platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "ktrouter");
  }
  return path.join(os.homedir(), ".ktrouter");
}
...
// Resolve npx/ktrouter binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx ktrouter` — works regardless of global bin path changes after npm i -g
